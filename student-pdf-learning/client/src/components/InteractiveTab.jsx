...
function InteractiveTab({ data, rerunState, onRerun, onVersionChange }) {
  ...
  const hasOriginalContent = !!(data?.concepts?.length > 0)

  if (!displayData || !displayData.concepts || displayData.concepts.length === 0) {
    return (
      <div className="text-center py-12 space-y-6">
        <div>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-kinesthetic/20 rounded-full mb-4 border border-kinesthetic/30">
          ...
          <h2 className="text-xl font-bold text-white font-heading">Interactive Learning</h2>
        ...
        <button
          ...
          className={`px-6 py-3 rounded-xl font-semibold transition-all duration-300 ${
            rerunState?.isLoading
              ? 'bg-kinesthetic/30 text-kinesthetic/70 cursor-not-allowed'
              : 'bg-gradient-to-r from-kinesthetic to-kinesthetic-600 text-white hover:shadow-kinesthetic-glow hover:scale-105'
          }`}
        >
          {rerunState?.isLoading ? (
            ...
            hasVersions ? 'Generate New Version' : 'Generate Interactive Content'
          )}
        </button>
        {rerunState?.error && (
