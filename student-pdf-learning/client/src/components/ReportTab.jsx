  ...
  if (!displayContent) {
    return (
      <div className="text-center py-12 space-y-6">
        <div>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-readwrite/20 rounded-full mb-4 border border-readwrite/30">
        ...
        <button
          ...
          className={`px-6 py-3 rounded-xl font-semibold transition-all duration-300 ${
            rerunState?.isLoading
              ? 'bg-readwrite/30 text-readwrite/70 cursor-not-allowed'
              : 'bg-gradient-to-r from-readwrite to-readwrite-600 text-white hover:shadow-readwrite-glow hover:scale-105'
          }`}
        >
          {rerunState?.isLoading ? (
            ...
            hasVersions ? 'Generate New Version' : 'Generate Study Report'
          )}
        </button>
        {rerunState?.error && (
