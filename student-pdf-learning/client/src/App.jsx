...
function AppContent() {
  const { user, isAuthenticated, becomeTeacher, guestLoginAsTeacher } = useAuth()
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {!isAuthenticated && (
                <button
                  onClick={async () => {
                    const result = await guestLoginAsTeacher()
                    if (result.success) {
                      setCurrentView('teacher')
                    }
                  }}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl font-medium transition-all duration-300 border bg-readwrite/20 text-readwrite border-readwrite/30 hover:bg-readwrite/30"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                  </svg>
                  <span className="hidden sm:inline">Try as Teacher</span>
                </button>
              )}
              <LoginButton />
            </div>
